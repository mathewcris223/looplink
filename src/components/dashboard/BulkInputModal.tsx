...
const BulkInputModal = ({ businessId, onClose, onSaved }: Props) => {
  ...
  const handleSave = async () => {
    const lowConfidence = rows.filter(r => r.confidence === "low" && (r.amount === null || !r.type));
    if (lowConfidence.length > 0) { setSaveError("Please fix the highlighted rows before saving."); return; }
    setSaving(true);
    setSaveError("");
    let saved = 0;
    try {
