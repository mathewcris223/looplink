...
const TruthEngine = ({ transactions, inventoryItems, inventorySales, businessId, businessType, businessName }: Props) => {
  ...
  useEffect(() => {
    ...
    // Fetch AI tip asynchronously
    const fetchTip = async () => {
      try {
        const income = transactions.filter(t => t.type === "income").reduce((s, t) => s + t.amount, 0);
        const expenses = transactions.filter(t => t.type === "expense").reduce((s, t) => s + t.amount, 0);
        const raw = await aiRequest({
          message: `Give me ONE actionable business tip in 15 words or less based on: today P&L ₦${computed.todayPnL.toLocaleString()}, weekly trend ${computed.weeklyTrendPct > 0 ? "+" : ""}${computed.weeklyTrendPct}%, top expense: ${computed.biggestExpenseCategory?.name ?? "none"}. Return ONLY the tip sentence, no quotes.`,
          businessType,
          businessName,
          totalIncome: income,
  ...
  const cards = [
    {
      icon: data.todayPnL >= 0 ? TrendingUp : TrendingDown,
      label: "Today's Result",
      value: `${data.todayPnL >= 0 ? "+" : ""}₦${data.todayPnL.toLocaleString()}`,
      sub: data.todayPnL >= 0 ? "Profit today" : "Loss today",
      color: pnlColor,
      cta: null,
    },
    {
      icon: data.weeklyTrendPct >= 0 ? TrendingUp : TrendingDown,
      label: "This Week",
      value: `${data.weeklyTrendPct > 0 ? "+" : ""}${data.weeklyTrendPct}%`,
      sub: data.weeklyTrendPct >= 0 ? "vs last week" : "vs last week",
      color: trendColor,
      cta: null,
    },
    {
      icon: Star,
      label: "Top Product",
      value: data.topProduct?.name ?? "No data",
      sub: data.topProduct ? `₦${data.topProduct.profit.toLocaleString()} profit (30d)` : "Record sales to see",
      color: data.topProduct ? "green" as const : "yellow" as const,
      cta: data.topProduct ? "/inventory" : null,
    },
    {
      icon: DollarSign,
      label: "Biggest Expense",
      value: data.biggestExpenseCategory?.name ?? "None",
      sub: data.biggestExpenseCategory ? `₦${data.biggestExpenseCategory.total.toLocaleString()} this month` : "No expenses yet",
      color: data.biggestExpenseCategory ? "red" as const : "green" as const,
      cta: null,
    },
    {
      icon: AlertTriangle,
      label: "Anomaly",
      value: data.anomaly ? data.anomaly.description : "All normal",
      sub: data.anomaly ? `₦${data.anomaly.amount.toLocaleString()} in ${data.anomaly.category}` : "No unusual spending",
      color: data.anomaly ? "yellow" as const : "green" as const,
      cta: null,
    },
    {
      icon: Lightbulb,
      label: "AI Tip",
      value: aiTip ?? "Loading tip…",
      sub: "Tap to chat with AI",
