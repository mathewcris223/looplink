  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "glass shadow-sm border-b" : "bg-transparent border-transparent"}`}>
      <div className="container mx-auto flex items-center justify-between h-16 px-6">
        ...
        <button className="md:hidden p-2 rounded-lg hover:bg-muted transition-colors" onClick={() => setOpen(!open)} aria-label="Toggle menu">
          {open ? <X size={22} /> : <Menu size={22} />}
        </button>
      </div>
      {open && (
        <div className="md:hidden glass border-t px-6 pb-5 pt-3 flex flex-col gap-1 animate-fade-up">
          <button onClick={() => { setOpen(false); navigate("/login"); }} className="py-2.5 text-sm text-muted-foreground hover:text-foreground transition-colors text-left">Log in</button>
          <Button variant="hero" size="sm" className="w-full mt-2 rounded-full" onClick={() => navigate("/signup")}>Get Started Free</Button>
        </div>
      )}
    </nav>
  );
