...
import { updateInventoryStock, InventoryItem } from "@/lib/db";
...
const RestockModal = ({ items, onClose, onSuccess, defaultItem }: Props) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await updateInventoryStock(selectedItemId, parseFloat(quantity));
      onSuccess();
      onClose();
    } catch (err: unknown) {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {selectedItem && (
            <div className="rounded-xl bg-muted/40 px-4 py-3 text-xs text-muted-foreground">
              Current stock: <span className="font-semibold text-foreground">{selectedItem.quantity ?? 0} {selectedItem.unit_type ?? "units"}</span>
            </div>
          )}
          <div>
            <label className="text-sm font-medium">
              Quantity to Add {selectedItem?.purchase_type === "pack" ? `(${selectedItem.pack_name ?? "packs"})` : ""}
            </label>
